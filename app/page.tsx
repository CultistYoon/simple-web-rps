...
interface AiPattern {
  ...
  playerWinChoices: Record<Choice, number>;
  aiLossStreak: number;
}
...
const SKINS = [
  { id: "default", name: "기본 손", levelRequired: 0, rarity: "common" },
  { id: "golden", name: "황금 손", levelRequired: 3, rarity: "rare" },
...
export default function RockPaperScissorsGame() {
  ...
  useEffect(() => {
    ...
    if (SHOW_PLAN_ON_FIRST_LOAD && !hasVisitedBefore) {
      ...
      localStorage.setItem("rps-has-visited", "true");
    }
  }, []);
  ...
  const playGame = (choice: Choice) => {
    ...
    setTimeout(() => {
      ...
      setAiPattern((prev) => {
        ...
        const updatedPlayerWinChoices = { ...prev.playerWinChoices, [choice]: prev.playerWinChoices[choice] + 1 };
        return {
          lastPlayerChoices: updatedLastPlayerChoices,
          playerWinChoices: updatedPlayerWinChoices,
          aiLossStreak: result === "lose" ? prev.aiLossStreak + 1 : 0,
        };
      });
      setShowResult(true);
      setIsPlaying(false);
        ...
        <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 p-4">
          <div className="max-w-4xl mx-auto">
            ...
            <Tabs defaultValue="game" className="space-y-6">
              <TabsList className="grid w-full grid-cols-4 bg-blue-100 p-1 rounded-xl shadow-inner">
                <TabsTrigger value="game" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md data-[state=active]:font-bold data-[state=active]:scale-105 transition-all duration-200 py-2 rounded-lg">게임</TabsTrigger>
                <TabsTrigger value="stats" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md data-[state=active]:font-bold data-[state=active]:scale-105 transition-all duration-200 py-2 rounded-lg">기록</TabsTrigger>
                <TabsTrigger value="shop" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md data-[state=active]:font-bold data-[state=active]:scale-105 transition-all duration-200 py-2 rounded-lg">스킨</TabsTrigger>
                <TabsTrigger value="settings" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md data-[state=active]:font-bold data-[state=active]:scale-105 transition-all duration-200 py-2 rounded-lg">설정</TabsTrigger>
              </TabsList>
              {/* 게임 탭 내용 */}
                      ...
                      <Card key={skin.id} className={`p-4 border-2 ${gameStats.selectedSkin === skin.id ? 'border-purple-500' : 'hover:border-blue-300'} transition-colors`}>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="text-right">
                            {skin.levelRequired > gameStats.level && (
                              <Badge className="bg-gray-500 text-white p-2">레벨 {skin.levelRequired} 이상 필요</Badge>
                            )}
                          </div>
                        </div>
                        <div className="flex gap-2">
                          {gameStats.unlockedSkins.includes(skin.id) ? (
                            <Button
                              ...
                              className="flex-1"
                              disabled={gameStats.level < skin.levelRequired}
                            >
                            ...
                            <Button
                              disabled={true}
                              className="flex-1 bg-gray-200 text-gray-600 font-semibold py-2 rounded-lg cursor-not-allowed"
                            >
                              해금 필요
                            </Button>
                          )}
